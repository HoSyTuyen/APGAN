...
# process.cpu_affinity(list(range(21,40)))
...
parser.add_argument('--con_lambda', type=float, default=180, help='lambda for content loss')
parser.add_argument('--is_edge', required=False, default='False')
...
parser.add_argument('--src_data', required=False, default='s_src_data',  help='sec data path')
parser.add_argument('--tgt_data', required=False, default='s_tgt_data',  help='tgt data path')
...
pretrain_loader_src = utils.data_load(os.path.join('data', args.src_data), 'pair', tgt_transform, args.batch_size, shuffle=True, drop_last=True)
...
if args.latest_generator_model == '':
    ...
    for epoch in range(args.pre_train_epoch):
        ...
        Recon_losses = []
        for x, _ in pretrain_loader_src:
            x_sm = x[:, :, :, args.input_size:]
            x = x[:, :, :, :args.input_size]

            # result = torch.cat((x[0], x_sm[0]), 2)
            # plt.imsave('test.png', (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
...
for epoch in range(args.train_epoch):
    ...
    for (x, _), (y, _) in zip(pretrain_loader_src, train_loader_tgt):
        ...
        # Con_loss = 10 * facenet.cosine_distance((x + 1) / 2, (G_ + 1) / 2)
